...
function getTextStyleAsJson (style, changes) {
	console.log("getTextStyleAsJson")
...
function setTypeStyle (context, style) {
  initVars(context);
  var size = style.size;
...
function updateTypeStyles(context, styleMap, desktopRamp) {
  console.log("updateTypeStyles")
  initVars(context)
  var ramp = "m";
  ...
  styleMap.forEach(function(calculatedStyle){
    ...
    sharedStyles.objects().forEach((documentStyle) => {
      // Get matching style
      console.log(String(documentStyle.name()).substring(0, String(token).length) + " == " + String(token).toUpperCase())
      if (String(documentStyle.name()).startsWith(String(token).toUpperCase())) {
        var style = getTextStyleAsJson(documentStyle, changes);
        setTypeStyle(context, style)
      }
    });
  });
}

function scaleTypeRamp(context){
  initVars(context)
  ...
  if (response === 1000) {
    ...
    if (desktopType && desktopType.length) {
      updateTypeStyles(context, desktopType, "desktop")
    }
    if (mobileType && mobileType.length) {
      updateTypeStyles(context, mobileType)
    }
  }
}
function onRun(context) {
	initVars(context)
	scaleTypeRamp(context)
}